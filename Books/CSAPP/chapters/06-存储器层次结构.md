...
<div style="text-align: center;"><img src="../raw_texts/imgs/img_in_image_box_491_73_697_490.jpg" alt="Image" width="10%" /></div>
...
实际上，内存系统是一个由不同容量、成本和访问时间的存储设备构成的层次结构。CPU 寄存器保存最常用的数据。CPU 附近的小型、快速高速缓存存储器充当主内存中部分数据和指令的暂存区，而主内存本身访问速度相对较慢。主内存是大型、慢速磁盘上数据的暂存区，而磁盘又常常充当通过网络连接的其他机器上的磁盘或磁带所存储数据的暂存区。
...
<div style="text-align: center;"><img src="../raw_texts/imgs/img_in_image_box_662_1979_1843_2362.jpg" alt="Image" width="60%" /></div>
...
<div style="text-align: center;">图 6.2 DRAM 和 SRAM 存储器的特性。</div>

## 动态 RAM

DRAM 将每个位作为电容器上的电荷存储。这个电容器非常小——通常约为 30 飞法拉，即 $30 \times 10^{-15}$ 法拉。不过，法拉是一个非常大的度量单位。DRAM 存储可以做得非常密集——每个单元由一个电容器和一个访问晶体管组成。然而，与 SRAM 不同，DRAM 存储单元对任何干扰都非常敏感。当电容器电压受到干扰时，它永远无法恢复。光线照射会导致电容器电压发生变化。事实上，数码相机和摄像机中的传感器本质上就是 DRAM 单元的阵列。
...
例如，图 6.3 显示了一个 $16 \times 8$ DRAM 芯片的组织结构，其中 $d = 16$ 个超单元，每个超单元 $w = 8$ 位，$r = 4$ 行，$c = 4$ 列。阴影框表示地址 $(2, 1)$ 处的超单元。信息通过称为引脚的外部连接器流入和流出芯片。每个引脚携带 1 位信号。图 6.3 显示了其中两组引脚：8 个数据引脚，可以传输 1 字节
...
<div style="text-align: center;">图 6.3 128 位 $16 \times 8$ DRAM 芯片的高层视图。</div>

<div style="text-align: center;"><img src="../raw_texts/imgs/img_in_image_box_673_494_1896_1125.jpg" alt="Image" width="62%" /></div>

进出芯片，以及两个携带 2 位行列超单元地址的 addr 引脚。其他携带控制信息的引脚未显示。
...
<div style="text-align: center;"><img src="../raw_texts/imgs/img_in_image_box_1_2_1851_632.jpg" alt="Image" width="94%" /></div>

<div style="text-align: center;">选择第 2 行（RAS 请求）。</div>

<div style="text-align: center;">(b) 选择第 1 列（CAS 请求）。</div>

<div style="text-align: center;">图 6.4 读取 DRAM 超单元的内容。</div>

## 内存模块

DRAM 芯片封装在内存模块中，插入主系统板（主板）上的扩展槽。Core i7 系统使用 240 引脚的双列直插式内存模块（DIMM），以 64 位为单位向内存控制器传输数据。
...
<div style="text-align: center;"><img src="../raw_texts/imgs/img_in_image_box_750_0_1954_993.jpg" alt="Image" width="61%" /></div>
...
**快速页模式 DRAM（FPM DRAM）**。传统 DRAM 将整行超单元复制到其内部行缓冲区，使用其中一个，然后丢弃其余的。FPM DRAM 通过允许对同一行的连续访问直接从行缓冲区提供服务来改善这一点。例如，要从传统 DRAM 的第 $i$ 行读取四个超单元，内存控制器必须发送四个 RAS/CAS 请求，即使每次的行地址 $i$ 相同。要从 FPM DRAM 的同一行读取超单元，内存控制器发送一个初始 RAS/CAS 请求，然后发送三个 CAS 请求。初始 RAS/CAS 请求将第 $i$ 行复制到行缓冲区，并返回由

---

<!-- Page 0614 -->

CAS 寻址的超单元。接下来的三个超单元直接从行缓冲区提供服务，因此比初始超单元返回得更快。
...
闪存是一种基于 EEPROM 的非易失性存储器，已成为重要的存储技术。闪存无处不在，为众多电子设备提供快速、耐用的非易失性存储，包括数码相机、手机、音乐播放器以及笔记本电脑、台式机和服务器计算机系统。在 6.1.3 节中，我们将详细介绍一种新型的基于闪存的磁盘驱动器，称为固态硬盘（SSD），它提供了比传统旋转磁盘更快、更坚固、更省电的替代方案。
...
数据通过称为**总线**的共享电气导线在处理器和 DRAM 主内存之间来回流动。CPU 和内存之间的每次数据传输都通过一系列步骤完成，这些步骤称为**总线事务**。读事务将数据从主内存传输到 CPU。写事务将数据从 CPU 传输到主内存。
...
图 6.6 展示了一个示例计算机系统的配置。主要组件是 CPU 芯片、我们称为 I/O 桥的芯片组（包括内存控制器）以及构成主内存的 DRAM 内存模块。这些组件通过一对总线连接：连接 CPU 和 I/O 桥的**系统总线**，以及连接 I/O 桥
...
<div style="text-align: center;"><img src="../raw_texts/imgs/img_in_image_box_577_713_1685_1211.jpg" alt="Image" width="56%" /></div>

与主内存的**内存总线**。I/O 桥将系统总线的电信号转换为内存总线的电信号。如我们将看到的，I/O 桥还将系统总线和内存总线连接到磁盘和显卡等 I/O 设备共享的 I/O 总线。但现在，我们将重点放在内存总线上。
...
<div style="text-align: center;">图 6.7 加载操作的内存读事务：movq A, %rax。</div>

<div style="text-align: center;"><img src="../raw_texts/imgs/img_in_image_box_635_0_1787_385.jpg" alt="Image" width="58%" /></div>

<div style="text-align: center;">(a) CPU 将地址 A 放到内存总线上。</div>

<div style="text-align: center;"><img src="../raw_texts/imgs/img_in_image_box_637_472_1809_892.jpg" alt="Image" width="59%" /></div>

<div style="text-align: center;">(b) 主内存从总线读取 A，取出字 x，并将其放到总线上。</div>

<div style="text-align: center;"><img src="../raw_texts/imgs/img_in_image_box_640_981_1852_1422.jpg" alt="Image" width="61%" /></div>

<div style="text-align: center;">(c) CPU 从总线读取字 x，并将其复制到寄存器 %rax。</div>
...
<div style="text-align: center;"><img src="../raw_texts/imgs/img_in_image_box_250_0_1466_411.jpg" alt="Image" width="61%" /></div>

<div style="text-align: center;">(a) CPU 将地址 A 放到内存总线上。主内存读取地址并等待数据字。</div>

<div style="text-align: center;"><img src="../raw_texts/imgs/img_in_image_box_232_504_1483_943.jpg" alt="Image" width="63%" /></div>

<div style="text-align: center;">(b) CPU 将数据字 y 放到总线上。</div>

<div style="text-align: center;"><img src="../raw_texts/imgs/img_in_image_box_226_1049_1482_1486.jpg" alt="Image" width="64%" /></div>

<div style="text-align: center;">(c) 主内存从总线读取数据字 y 并将其存储到地址 A。</div>

<div style="text-align: center;">图 6.8 存储操作的内存写事务：movq %rax, A。</div>
...
<div style="text-align: center;"><img src="../raw_texts/imgs/img_in_image_box_44_1_1246_573.jpg" alt="Image" width="61%" /></div>

<div style="text-align: center;"><img src="../raw_texts/imgs/img_in_image_box_1285_0_1961_432.jpg" alt="Image" width="34%" /></div>

<div style="text-align: center;">(a) 单盘片视图</div>

<div style="text-align: center;">(b) 多盘片视图</div>

<div style="text-align: center;">图 6.9 磁盘几何结构。</div>
...
**面密度（bits/in$^{2}$）**。记录密度与磁道密度的乘积。

磁盘制造商不懈地努力提高面密度（以及容量），该值每隔几年就翻一番。最初设计于面密度较低时代的磁盘，将每条磁道划分为相同数量的扇区，该数量由最内层磁道上可记录的扇区数决定。为了保持每条磁道的扇区数固定，外层磁道上的扇区间距更大。这在面密度相对较低时是合理的方法，
...
然而，随着面密度的提高，扇区之间的间隙（不存储数据位的地方）变得不可接受地大。因此，现代大容量磁盘使用一种称为**多区域记录（multiple zone recording）**的技术，其中柱面集合被划分为称为**记录区域（recording zone）**的不相交子集。每个区域由连续的柱面集合组成。每个区域中每个柱面的每条磁道具有相同数量的扇区，该数量由可以压入该区域最内层磁道的扇区数决定。
...
磁盘使用连接到**传动臂（actuator arm）**末端的**读/写头（read/write head）**来读写存储在磁性表面的位，如图 6.10(a) 所示。通过

---

<!-- Page 0621 -->

<div style="text-align: center;"><img src="../raw_texts/imgs/img_in_image_box_89_2_1367_625.jpg" alt="Image" width="65%" /></div>

读/写头连接在传动臂末端，在磁盘表面上方的薄空气垫上飞行。

通过径向移动，传动臂可以将读/写头定位在任意磁道上。

<div style="text-align: center;"><img src="../raw_texts/imgs/img_in_image_box_1424_2_1962_489.jpg" alt="Image" width="27%" /></div>

<div style="text-align: center;">(a) 单盘片视图</div>

<div style="text-align: center;">(b) 多盘片视图</div>

<div style="text-align: center;">图 6.10 磁盘动态特性。</div>

沿径向轴前后移动传动臂，驱动器可以将读/写头定位在表面的任意磁道上。这种机械运动称为**寻道（seek）**。一旦读/写头定位在目标磁道上，当磁道上的每个位依次经过读/写头时，读/写头可以感知位的值（读位）或改变位的值（写位）。有多个盘片的磁盘为每个表面配备一个单独的读/写头，如图 6.10(b) 所示。读/写头垂直对齐并同步移动。在任意时刻，所有读/写头都定位在同一柱面上。
...
**寻道时间**。要读取某个目标扇区的内容，传动臂首先将读/写头定位在包含目标扇区的磁道上。移动传动臂所需的时间称为**寻道时间**。寻道时间 $T_{\text{seek}}$ 取决于读/写头之前的位置和传动臂在表面移动的速度。现代驱动器中的平均寻道时间 $T_{\text{avg seek}}$，通过对数千次随机扇区寻道取均值来测量，通常约为 3 到 9 毫秒。单次寻道的最大时间 $T_{\text{max seek}}$ 可高达 20 毫秒。

---

<!-- Page 0622 -->

等待目标扇区的第一位经过读/写头所需的时间。这一步骤的性能取决于读/写头到达目标磁道时表面的位置以及磁盘的转速。在最坏情况下，读/写头刚好错过目标扇区，需要等待磁盘完整旋转一圈。因此，最大旋转延迟（以秒为单位）为：

$$T_{\text{max rotation}} = \frac{1}{\text{RPM}} \times \frac{60\ \text{秒}}{1\ \text{分钟}}$$

平均旋转延迟 $T_{\text{avg rotation}}$ 简单地等于 $T_{\text{max rotation}}$ 的一半。

**传输时间**。当目标扇区的第一位在读/写头下方时，驱动器即可开始读写扇区内容。一个扇区的传输时间取决于转速和每条磁道的扇区数。因此，我们可以粗略估算一个扇区的平均传输时间（以秒为单位）为：
...
<div style="text-align: center;">图 6.11 连接 CPU、主内存和 I/O 设备的示例总线结构。</div>

<div style="text-align: center;"><img src="../raw_texts/imgs/img_in_image_box_653_0_1962_1208.jpg" alt="Image" width="66%" /></div>
...
<div style="text-align: center;"><img src="../raw_texts/imgs/img_in_image_box_582_0_1499_630.jpg" alt="Image" width="46%" /></div>

<div style="text-align: center;">(a) CPU 通过向与磁盘关联的内存映射地址写入命令、逻辑块编号和目标内存地址来发起磁盘读取。</div>

<div style="text-align: center;">CPU 芯片</div>

<div style="text-align: center;"><img src="../raw_texts/imgs/img_in_image_box_526_776_1516_1523.jpg" alt="Image" width="50%" /></div>

<div style="text-align: center;">(b) 磁盘控制器读取扇区并执行 DMA 传输到主内存。</div>

<div style="text-align: center;">CPU 芯片</div>

<div style="text-align: center;"><img src="../raw_texts/imgs/img_in_image_box_500_1633_1501_2363.jpg" alt="Image" width="51%" /></div>

<div style="text-align: center;">(c) 磁盘控制器发送中断信号通知 CPU DMA 传输已完成。</div>
...
>**旁注 一款商用磁盘驱动器的特性**
>
>磁盘制造商在其网站上发布了大量有用的高层技术信息。例如，希捷（Seagate）网站包含以下关于其热门产品 Barracuda 7400 的信息（以及更多！）。
...
<div style="text-align: center;"><img src="../raw_texts/imgs/img_in_image_box_564_848_1924_1408.jpg" alt="Image" width="69%" /></div>
...
图 6.14 展示了典型 SSD 的性能特性。注意，从 SSD 读取比写入更快。随机读写性能之间的差异由底层闪存的基本特性造成。如图 6.13 所示，闪存由 $B$ 个块的序列组成，每个块由 $P$ 个页面组成。通常，页面大小为 512 字节到 4 KB，一个块由 32 到 128 个页面组成，总块大小范围从 16
...
<div style="text-align: center;">图 6.14 商用固态硬盘的性能特性。资料来源：Intel SSD 730 产品规格 [53]。IOPS 为每秒 I/O 操作次数。吞吐量数据基于读写 4 KB 块。</div>

KB 到 512 KB 不等。数据以页面为单位读写。一个页面只有在其所属的整个块被擦除后才能写入（通常，这意味着块中所有位被设置为 1）。然而，一旦块被擦除，块中的每个页面可以写入一次而无需进一步擦除。一个块在大约 100,000 次重复写入后会磨损报废，一旦磨损就无法再使用。
...
**练习题 6.5（答案见第 698 页）** 如我们所见，SSD 的一个潜在缺点是底层闪存可能磨损。例如，对于图 6.14 中的 SSD，Intel 保证在驱动器磨损之前大约可以写入

---

<!-- Page 0630 -->

128 拍字节（$128 \times 10^{15}$ 字节）的数据。基于这个假设，估算该 SSD 在以下工作负载下的使用寿命（年）：
...
<div style="text-align: center;">(d) CPU 趋势</div>

<div style="text-align: center;">图 6.15 存储和处理技术趋势。2010 年前后的 Core i7 使用 Nehalem 工艺核心；2015 年前后的 Core i7 使用 Haswell 核心。</div>

CPU 性能曲线在 2003 年前后的分叉反映了多核处理器的引入（见第 641 页的旁注）。在此分叉之后，单个核心的时钟周期时间实际上先略有增加，然后才再次开始下降，但下降速率比以前慢。

注意，SRAM 性能虽然落后，但大体上跟得上。然而，DRAM 和磁盘性能与 CPU 性能之间的差距实际上在不断扩大。直到 2003 年前后多核处理器问世之前，这一性能差距主要是延迟的函数，DRAM 和磁盘访问时间的降低速度慢于单个处理器的周期时间。然而，随着多核的引入，这一性能差距越来越成为

---

<!-- Page 0632 -->

<div style="text-align: center;"><img src="../raw_texts/imgs/img_in_chart_box_12_1_1629_717.jpg" alt="Image" width="82%" /></div>

<div style="text-align: center;">图 6.16 磁盘、DRAM 和 CPU 速度之间的差距。</div>

吞吐量的函数，多个处理器核心并行向 DRAM 和磁盘发出请求。
...
**练习题 6.6（答案见第 698 页）** 使用图 6.15(c) 中 2005 年至 2015 年的数据，估算何时能以 200 美元购买 1 拍字节（$10^{15}$ 字节）的旋转磁盘存储。假设使用实际美元（不考虑通货膨胀）。
...
局部性通常被描述为具有两种不同形式：**时间局部性（temporal locality）**和**空间局部性（spatial locality）**。在具有良好时间局部性的程序中，被引用一次的内存位置很可能在不久的将来再次被引用多次。在具有良好空间局部性的程序中，如果一个内存位置被引用了，
...
<div style="text-align: center;">图 6.17 (a) 一个局部性良好的函数。(b) 向量 v 的引用模式（N = 8）。注意向量元素的访问顺序与其在内存中存储的顺序相同。</div>
...
<div style="text-align: center;">图 6.19 (a) 一个空间局部性差的函数。(b) 数组 a 的引用模式（M = 2，N = 3）。该函数空间局部性差，因为它以步幅-N 引用模式扫描内存。</div>
...
由于程序指令存储在内存中且必须由 CPU 取出（读取），我们也可以评估程序关于其指令取出的局部性。例如，在图 6.17 中，

---

<!-- Page 0636 -->

for 循环体中的指令按顺序内存顺序执行，因此该循环具有良好的空间局部性。由于循环体被多次执行，它也具有良好的时间局部性。
...
**练习题 6.7（答案见第 698 页）** 重新排列以下函数中的循环，使其以步幅-1 引用模式扫描三维数组 a。
...
**练习题 6.8（答案见第 699 页）** 图 6.20 中的三个函数执行相同的操作，但空间局部性程度不同。按空间局部性对这些函数进行排名，并解释你得出排名的理由。
...
<div style="text-align: center;"><img src="../raw_texts/imgs/img_in_image_box_175_0_1899_955.jpg" alt="Image" width="87%" /></div>

<div style="text-align: center;">图 6.21 存储器层次结构。</div>
...
存储器层次结构的核心思想是，对于每个层次 $k$，层次 $k$ 处更快更小的存储设备充当层次
...
<div style="text-align: center;"><img src="../raw_texts/imgs/img_in_image_box_88_737_1550_1460.jpg" alt="Image" width="74%" /></div>

<div style="text-align: center;">图 6.22 存储器层次结构中缓存的基本原理。</div>

$k+1$ 处较大较慢存储设备的高速缓存。换言之，层次结构中的每一层都缓存来自下一个较低层的数据对象。例如，本地磁盘充当通过网络从远程磁盘检索的文件（如 Web 页面）的高速缓存，主内存充当本地磁盘上数据的高速缓存，以此类推，直到我们达到最小的高速缓存——CPU 寄存器集。
...
类似地，层次 $k$ 的存储被划分为一组更小的块，其大小与层次 $k+1$ 的块相同。在任意时刻，层次 $k$ 的高速缓存包含来自层次 $k+1$ 的块子集的副本。例如，在

---

<!-- Page 0640 -->

图 6.22 中，层次 $k$ 的高速缓存可以容纳四个块，目前包含块 4、9、14 和 3 的副本。
...
<div style="text-align: center;">图 6.23 现代计算机系统中缓存的普遍性。缩写：TLB：转换后备缓冲区；MMU：内存管理单元；OS：操作系统；NFS：网络文件系统。</div>
...
早期计算机系统的存储器层次结构仅由三个层次组成：CPU 寄存器、主内存和磁盘存储。然而，由于 CPU 和主内存之间差距的不断扩大，系统设计者不得不插入

---

<!-- Page 0643 -->

<div style="text-align: center;">图 6.24</div>

高速缓存存储器的典型总线结构。

<div style="text-align: center;"><img src="../raw_texts/imgs/img_in_image_box_705_7_1959_519.jpg" alt="Image" width="63%" /></div>

一个小型 SRAM 高速缓存存储器，称为 **L1 高速缓存（一级高速缓存）**，位于 CPU 寄存器文件和主内存之间，如图 6.24 所示。L1 高速缓存的访问速度几乎与寄存器一样快，通常约为 4 个时钟周期。
...
参数 $S$ 和 $B$ 将 $m$ 个地址位划分为图 6.25(b) 所示的三个字段。$A$ 中的 $s$ 个**组索引位（set index bit）**形成一个索引
...
(a) 高速缓存是一个组的数组。每个组包含一行或多行。每行包含一个有效位、一些标记位和一个数据块。(b) 高速缓存组织将 $m$ 个地址位划分为 $t$ 个标记位、$s$ 个组索引位和 $b$ 个块偏移位。

<div style="text-align: center;"><img src="../raw_texts/imgs/img_in_image_box_524_3_1848_814.jpg" alt="Image" width="67%" /></div>

<div style="text-align: center;">(a)</div>

<div style="text-align: center;"><img src="../raw_texts/imgs/img_in_image_box_509_884_1281_1109.jpg" alt="Image" width="39%" /></div>

<div style="text-align: center;">(b)</div>

指向组数组。第一个组是组 0，第二个组是组 1，依此类推。当解释为无符号整数时，组索引位告诉我们该字必须存储在哪个组中。一旦我们知道该字必须包含在哪个组中，$A$ 中的标记位告诉我们组中的哪一行（如果有的话）包含该字。当且仅当有效位被设置且行中的标记位与地址 $A$ 中的标记位匹配时，该组中的一行包含该字。一旦我们找到了由组索引标识的组中由标记标识的行，$b$ 个**块偏移位（block offset bit）**给出字在 $B$ 字节数据块中的偏移量。

正如你可能注意到的，高速缓存的描述使用了很多符号。图 6.26 总结了这些符号以供参考。

**练习题 6.9（答案见第 699 页）** 下表给出了多个不同高速缓存的参数。对于每个高速缓存，确定高速缓存组数 $(S)$、标记位数 $(t)$、组索引位数 $(s)$ 和块偏移位数 $(b)$。
...
<div style="text-align: center;">图 6.26 高速缓存参数摘要。</div>

<div style="text-align: center;">图 6.27 直接映射高速缓存（$E = 1$）。每组恰好有一行。</div>

<div style="text-align: center;"><img src="../raw_texts/imgs/img_in_image_box_679_1008_1888_1385.jpg" alt="Image" width="61%" /></div>
...
<div style="text-align: center;">图 6.28 直接映射高速缓存中的组选择。</div>

<div style="text-align: center;"><img src="../raw_texts/imgs/img_in_image_box_468_1_1886_432.jpg" alt="Image" width="72%" /></div>

### 图 6.29

直接映射高速缓存中的行匹配和字选择。在高速缓存块中，$w_0$ 表示字 $w$ 的低位字节，$w_1$ 表示下一个字节，依此类推。

<div style="text-align: center;"><img src="../raw_texts/imgs/img_in_image_box_571_574_1839_1085.jpg" alt="Image" width="64%" /></div>
...
<div style="text-align: center;">图 6.30 直接映射高速缓存示例的 4 位地址空间。</div>
...
5. **再次读取地址 0 处的字**。这又是一次不命中，原因是我们刚刚在读取地址 8 时替换掉了块 0。这类不命中是一个冲突不命中 (conflict miss) 的典型示例：高速缓存中明明还有空位，却因为反复交替访问映射到同一组的块而导致不命中。
...
在运行时，循环第一次迭代引用 `x[0]`，发生不命中，导致包含 `x[0]`–`x[3]` 的块被装入组 0。下一次引用 `y[0]` 又不命中，包含 `y[0]`–`y[3]` 的块被复制到组 0，覆盖了之前装入的 `x` 的值。在下一次迭代中，对 `x[1]` 的引用又不命中，`x[0]`–`x[3]` 的块被重新装入组 0，覆盖了 `y[0]`–`y[3]` 的块。至此产生了冲突不命中，实际上后续对 `x` 和 `y` 的每次引用都会导致冲突不命中，我们在 `x` 和 `y` 的块之间反复抖动 (thrashing)。**抖动**描述的是高速缓存反复装入和驱逐同一组高速缓存块的任何情形。
...
<div style="text-align: center;"><img src="../raw_texts/imgs/img_in_image_box_60_637_1661_1800.jpg" alt="Image" width="81%" /></div>

<div style="text-align: center;"><img src="../raw_texts/imgs/img_in_chart_box_662_649_1657_1803.jpg" alt="Image" width="50%" /></div>

<div style="text-align: center;">图 6.31 为什么高速缓存使用中间位作为索引。</div>
...
<div style="text-align: center;">图 6.32</div>

组相联高速缓存 $(1 < E < C/B)$。在组相联高速缓存中，每个组包含多条行。本例展示了一个二路组相联高速缓存。

<div style="text-align: center;"><img src="../raw_texts/imgs/img_in_image_box_583_0_1959_1304.jpg" alt="Image" width="70%" /></div>

<div style="text-align: center;">图 6.33 组相联高速缓存中的组选择。</div>
...
<div style="text-align: center;"><img src="../raw_texts/imgs/img_in_image_box_513_4_1817_560.jpg" alt="Image" width="66%" /></div>

<div style="text-align: center;">图 6.34 组相联高速缓存中的行匹配与字选择。</div>
...
<div style="text-align: center;"><img src="../raw_texts/imgs/img_in_image_box_622_0_1954_1591.jpg" alt="Image" width="67%" /></div>

图 6.37：全相联高速缓存中的行匹配与字选择。

（3）若（1）和（2）均满足，则命中，块偏移选择起始字节。
...
由于高速缓存电路必须并行搜索大量匹配的标记，构建既大又快的相联高速缓存十分困难且代价高昂。因此，全相联高速缓存只适合用于小型高速缓存，例如

---

<!-- Page 0656 -->

虚拟内存系统中用于缓存页表项的转换后备缓冲器 (Translation Lookaside Buffer, TLB)（第 9.6.2 节）。
...
<div style="text-align: center;">二路组相联高速缓存</div>
...
假设在练习题 6.12 的机器上运行某程序，该程序访问地址 0x0D53 处的 1 字节字。请指出所访问的高速缓存条目及该高速缓存字节
...
返回值以十六进制表示。指出是否发生高速缓存不命中。若不命中，在"Cache byte returned"中填入"—"。
...
<div style="text-align: center;"><img src="../raw_texts/imgs/img_in_image_box_144_2264_1636_2405.jpg" alt="Image" width="76%" /></div>
...
针对写操作优化高速缓存是一个微妙而困难的问题，这里仅作初步介绍。具体细节因系统而异，往往是专有信息且文档不完整。对于试图编写

---

<!-- Page 0659 -->

高效高速缓存友好程序的程序员，我们建议采用**写回、写分配**高速缓存的思维模型，理由如下：通常情况下，存储器层次结构中较低层次的高速缓存更倾向于使用写回而非直写，因为较低层的传输时间更长。例如，虚拟内存系统（以主内存作为磁盘上存储块的高速缓存）完全采用写回策略。随着逻辑密度的提高，写回的复杂性正在变得越来越不成问题，现代系统的各个层次都在使用写回高速缓存。因此，该假设与当前趋势相符。另一个采用写回、写分配假设的原因是它与读操作的处理方式具有对称性——写回、写分配同样试图利用局部性。这样，我们就可以从高层次出发，编写具有良好空间局部性和时间局部性的程序，而不必针对特定的内存系统进行优化。
...
<div style="text-align: center;"><img src="../raw_texts/imgs/img_in_image_box_583_3_1807_1052.jpg" alt="Image" width="62%" /></div>
...
<div style="text-align: center;">图 6.39 Intel Core i7 高速缓存层次结构的特性。</div>
...
在 6.2 节中，我们介绍了局部性的概念，并从定性角度探讨了什么构成良好的局部性。现在我们理解了高速缓存的工作原理，可以更精确地加以描述。具有较好局部性的程序往往具有较低的不命中率，而不命中率较低的程序往往比不命中率较高的程序运行更快。因此，优秀的程序员应当始终努力
...
编写高速缓存友好的代码，即具有良好局部性的代码。下面是我们用来确保代码高速缓存友好的基本方法。
...
访问序号      1[m]   2[h]   3[h]   4[h]   5[m]   6[h]   7[h]   8[h]
```
...
<table border=1 style='margin: auto; word-wrap: break-word;'><tr><td colspan="3">dst array</td><td colspan="3">src array</td></tr><tr><td style='text-align: center; word-wrap: break-word;'></td><td style='text-align: center; word-wrap: break-word;'>Col. 0</td><td style='text-align: center; word-wrap: break-word;'>Col. 1</td><td style='text-align: center; word-wrap: break-word;'></td><td style='text-align: center; word-wrap: break-word;'>Col. 0</td><td style='text-align: center; word-wrap: break-word;'>Col. 1</td></tr><tr><td style='text-align: center; word-wrap: break-word;'>Row 0</td><td style='text-align: center; word-wrap: break-word;'>m</td><td style='text-align: center; word-wrap: break-word;'>____</td><td style='text-align: center; word-wrap: break-word;'>Row 0</td><td style='text-align: center; word-wrap: break-word;'>m</td><td style='text-align: center; word-wrap: break-word;'>____</td></tr><tr><td style='text-align: center; word-wrap: break-word;'>Row 1</td><td style='text-align: center; word-wrap: break-word;'>____</td><td style='text-align: center; word-wrap: break-word;'>____</td><td style='text-align: center; word-wrap: break-word;'>Row 1</td><td style='text-align: center; word-wrap: break-word;'>____</td><td style='text-align: center; word-wrap: break-word;'>____</td></tr></table>
...
<div style="text-align: center;">code/mem/mountain/mountain.c</div>

<div style="text-align: center;">图 6.40 测量并计算读吞吐量的函数。通过以不同的 size（对应时间局部性）和 stride（对应空间局部性）调用 run 函数，可以生成特定计算机的存储器山。</div>

---

<!-- Page 0669 -->

<div style="text-align: center;"><img src="../raw_texts/imgs/img_in_chart_box_0_2_1962_1246.jpg" alt="Image" width="100%" /></div>

<div style="text-align: center;">图 6.41 存储器山。展示了读吞吐量关于时间局部性和空间局部性的函数。</div>
...
<div style="text-align: center;"><img src="../raw_texts/imgs/img_in_chart_box_260_0_1799_957.jpg" alt="Image" width="78%" /></div>

<div style="text-align: center;">图 6.42 存储器山中的时间局部性山脊。该图展示了图 6.41 中步长 = 8 时的切片。</div>
...
<div style="text-align: center;"><img src="../raw_texts/imgs/img_in_chart_box_0_2_1267_774.jpg" alt="Image" width="64%" /></div>

<div style="text-align: center;">图 6.43 空间局部性坡面。该图展示了图 6.41 中 size = 4 MB 时的切片。</div>
...
从高层次来看，六个版本相当类似：若加法满足结合律，则每个版本计算出相同的结果$^1$。每个版本共执行 $O(n^3)$ 次操作，且加法和乘法次数相同；$A$ 和 $B$ 中每个 $n^2$ 个元素各被读取 $n$ 次；$C$ 的每个 $n^2$ 个元素通过对 $n$ 个值求和得到。然而，若分析最内层循环迭代的行为，会发现各版本在访问次数和局部性方面存在差异。为便于分析，我们作如下假设：
...
AC 类例程（图 6.44(c) 和 (d)）的内层循环存在一些问题：每次迭代执行两次 load 和一次 store（相比之下，
...
<div style="text-align: center;">图 6.44 矩阵乘法的六个版本。每个版本由其循环顺序唯一标识。</div>
...
<div style="text-align: center;">图 6.45 矩阵乘法内层循环分析。六个版本分为三个等价类，以内层循环中访问的矩阵对命名。</div>

---

<!-- Page 0674 -->

<div style="text-align: center;"><img src="../raw_texts/imgs/img_in_chart_box_216_0_1681_831.jpg" alt="Image" width="74%" /></div>

<div style="text-align: center;">图 6.46 Core i7 矩阵乘法性能。</div>

AB 类例程每次迭代执行两次 load、无 store）。此外，内层循环以步长 $n$ 扫描 $A$ 和 $C$ 的列，导致每次 load 都不命中，共计每迭代 2 次不命中。注意，循环顺序的调换降低了与 AB 类例程相比的空间局部性。
...
- 在本例中，不命中率比总内存访问次数更能预测性能。例如，每迭代 0.5 次不命中的 BC 类例程，性能远好于每迭代 1.25 次不命中的 AB 类例程，尽管 BC 类例程在内层循环中执行了更多
...
内存引用（两次 load 和一次 store）比 AB 类例程（两次 load）更多。
...
Stricker 在 [112] 中引入了存储器山的概念，作为存储器系统的综合表征，并在后续的工作报告中非正式地提出了"存储器山"这一术语。编译器研究人员
...
通过自动执行我们在第 6.6 节中讨论的那些手动代码变换，研究人员已做了大量工作来提高局部性 [22, 32, 66, 72, 79, 87, 119]。Carter 等人提出了一种高速缓存感知内存控制器 [17]。其他研究人员开发了**高速缓存无关 (cache-oblivious)** 算法，这类算法无需显式了解底层高速缓存结构即可高效运行 [30, 38, 39, 9]。
...
CO：高速缓存块偏移；CI：高速缓存组索引；CT：高速缓存标记
...
<div style="text-align: center;">四路组相联高速缓存</div>
...
<div style="text-align: center;"><img src="../raw_texts/imgs/img_in_chart_box_386_1874_1764_2001.jpg" alt="Image" width="70%" /></div>
...
<div style="text-align: center;">图 6.47 习题 6.37 中引用的函数。</div>
...
<div style="text-align: center;">B. 当高速缓存为 32 字节时，它足以容纳两个数组。因此，唯一的不命中是初始的冷不命中。</div>
...
<div style="text-align: center;">图 6.48</div>

<div style="text-align: center;">习题 6.17 的解答示意图。</div>

<div style="text-align: center;"><img src="../raw_texts/imgs/img_in_chart_box_669_2220_1755_2412.jpg" alt="Image" width="55%" /></div>
